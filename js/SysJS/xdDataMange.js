 ...
 * @描述: 
 * @版本: 1.00
 * @修改历史纪录: （加header 修改xklx）
 * @记录: 1. 10/14 马娜 更换登录接口
 */
//地址
var httpIp="http://192.168.28.251:8111/"
var ip_url = httpIp+"/api/kf/data";
var ip_url_user = httpIp+"/wwSYGR/wwData/wwDataManageGR.ashx"; //请求角色部分的地址
...
function getCookieName(name) {
  ...
  if (arr = document.cookie.match(reg)) {
    ...
    if (isQcode) {
      ...
      parent.location.href = "/rsgl/login.html"
    }
...
//						if(limitNumber==true){
//							if($(demoListView).find("tr").length==6){
//							layer.msg("最多上传6张图片")
...
//						}
    ...
    that_.postData(xation, objData, function (data) {
      ...
      for (var i = 0; i < data.data.length; i++) {
        ...
        html += `
			...
			<td>
			<button class="layui-btn layui-btn-xs demo-reload layui-hide">重传</button>
			<span class="layui-btn layui-btn-xs layui-btn-danger " id="delete${i}" onclick="deleteFile('${data.data[i][newName]}','${type2 + i}','','${data.data[i].id}')">删除</span>
  ...
  postData: function (mActionType, mActionData, callback, _url, async) {
    ...
    if (mActionType) {
      url_ = url_ + "?XAction=" + mActionType + "&sykf=KF_SC"
    }
    var async = true;
    if (async) {
      async = async;
  ...
  if (pictureType == "03") {
    $.ajax({
      ...
      success: function (dataxml) {
        ...
        url = "/kf/Widget/measurablePicture/openTitleImage.html?h=" + heigh_ + "&w=" + width_ + "&path=" + imgPath
      },
      error: function () {

        url = '/kf/Widget/imgTools/ShowImage.html?path=' + imgPath;
        //				url = "/kf/Widget/measurablePicture/openTitleImage.html?h=" + "10000" + "&w=" + "1000" + "&path=" + "TP0000002/TP0000002/"
      }
    });

  } else if (pictureType == "02") {
    url = '/kf/widget/pictureDetail/showPicture.html?path=' + imgPath;
...
function ShowVideo(mtitle, mpath, w, h, clobtn, system) {
  if (mpath == '') {
    ...
    var url = "/kf/widget/video/ShowVideo.html?path=" + mpath;
    switch (r.toLowerCase()) {
      ...
      case "xlsx":
        break;
      case "pdf":
        url = '/kf/widget/pdfViewer/pdfView.html?path=' + mpath;
        yl = true;
        break;
      case "png":
      case "jpg":
      ...
      case "swf":
      case "mp5":
        url = "/kf/widget/video/ShowVideo.html?path=" + mpath;
        yl = true;
        break;
      default:
        yl = false;
              ...
              success: function () {
                if (data.success || data.msg) {
                  ...
                  var layer003 = layer.confirm("是否打印" + tip + "单?", {
                    ...
                    {
                      if (tip == "出库") {
                        printAllList(knbh_, data.log_codes, "出库单") //data['XDLM记录表流水号']
                      } else {
                        printAllList(knbh_, data.log_codes, "归库单") //data['XDLM记录表流水号']
                      }
...
function printAllList(ids, lsNumber, type) {
...
//		}, function() {
    var index3 = layer.msg('正在导出,请稍等...', {
      ...
      success: function (index, layero) {
        ...
        postData("WordPrint", where, function (returnData) {
          if (returnData.success || returnData.msg) {
            layer.msg('导出完成', {
              time: 500,
              icon: 1
