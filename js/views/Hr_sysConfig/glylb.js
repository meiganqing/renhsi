      ...
      switch (obj.event) {
        case "edit":
          if (limitConfig("storeLimt_rsgl", 2)) {
            layerPage01 = openWindow(2, 'addUser.html?id=' + data.id, "编辑用户", 600, 670);
          } else {
            layer.msg("您没有编辑权限")
          }
...
//        }
        ...
        case "setting":
//        if (limitConfig("storeLimt_rsgl", 2)) {
            layerPage01 = openWindow(2, "xdUserRole.html?onlynum=" + data.onlynum, "编辑用户", 600, 700);
//        } else {
//          layer.msg("您没有编辑权限")
//        }
    ...
    //添加管理员
    $("#add").click(function () {
      layerPage01 = openWindow(2, 'addUser.html', "编辑用户", 600, 500);
      form.render("select")
...
function getTable(id, where, type, action, height) {
  ...
  layui.use(["table"], function () {
    ...
    tableins = table.render({
      elem: '#' + id,
      url: "" + ip_url + "?sykf=syyh&XKLX=SYYHGL&XAction=" + xAction,
      where: where,
      headers: {
