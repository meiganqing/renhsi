	...
	layui.use(["form", "table"], function() {
		...
		table.on('tool(demo)', function(obj) {
          ...
          if (limitConfig("storeLimt_rsgl", 2)) {
            layerPage01 = openWindow(2, 'add_rolesList.html?id=' + data.id, "编辑", 500, 240);
          }else{
            layer.msg('您没有编辑权限');
          }
					break;
				case "setting":
          if (limitConfig("storeLimt_rsgl", 2)) {
            layerPage01 = openWindow(2, 'config_xdItemList.html?id=' + data.id + '&onlynum=' + data.roleNum + '&rolename=' + escape(data.rolename) + '&miaoshu=' + escape(data.miaoshu), "配置信息", $(window).width() - 100, $(window).height() - 100);
          }else{
            layer.msg('您没有配置权限');
          }
					break;
		...
		$("#addData").click(function() {
			...
			if(type == "role") {
				layerPage01 = openWindow(2, 'add_rolesList.html', "详细信息", 500, 240);
...
function getTable_(id, where, type, action, height) {
	...
	layui.use(["table"], function() {
		...
		tableins = table.render({
			elem: '#' + id,
			url: "/api/kf/data?sykf=syyh&XKLX=SYYHGL&XAction=" + xAction,
			// url: "http://192.168.28.251:8111/xdGetData/DataHandler.ashx?sykf=syyh&XKLX=syyh&XAction=" + xAction,
			where: where,
			headers: {
