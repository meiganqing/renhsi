...
layui.extend({
    ...
    //请求函数
    function postDataMenu(title, url) { //封装页面请求函数
        try {
            $.ajax({
                async: false,
                cache: false,
                type: "post",
                url: "/xdData/xdDataManage.ashx?XAction=GetDataInterface&XKLX=SYYHGL",
                data: {
                    XDLMCID: "5000",
                    XDLMSID: "DYBH2019082310260126120333",
                    XDLMsystem: "科研管理系统",
                    XDLMurl: url,
                    XDLMuser_name: getCookieName("mUserName"),
                    XDLMname: title,
                    XDLMcreated_at: currentDate() + " " + currentTime()
                },
                dataType: 'json',
                headers: {
                    Authorization: getAuth() //登录后返回sytoken
                },
